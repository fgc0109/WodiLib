...
namespace WodiLib.Test.Sys
{
    ...
    public class WoditorStringTest
    {
        ...
        public static void FromByteArrayTest(byte[] value, string result, long offset)
        {
            ...
            Assert.AreEqual(result, instance.String);
            Assert.AreEqual(value.Length - offset, instance.ByteLength);

            var instanceStringBytes = instance.StringByte.ToArray();
            for (var i = 0; i < instance.ByteLength; i++)
            {
                Assert.AreEqual(value[offset + i], instanceStringBytes[i]);
            }
        }
        ...
        public static void FromStringTest(byte[] result, string value, long offset)
        {
            ...
            Assert.AreEqual(value, instance.String);
            Assert.AreEqual(result.Length - offset, instance.ByteLength);

            var instanceStringBytes = instance.StringByte.ToArray();
            for (var i = 0; i < instance.ByteLength; i++)
            {
                Assert.AreEqual(result[offset + i], instanceStringBytes[i]);
            }
        }
    }
}

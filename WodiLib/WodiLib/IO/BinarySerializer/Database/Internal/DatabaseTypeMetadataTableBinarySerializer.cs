...
// File Name    : DatabaseTypeMetadataTableBinarySerializer.cs
//
...
{
    /// <summary>
    ///     <see cref="ReadOnlyDatabaseTypeMetadataTable"/> をバイナリ配列に変換するためのHelperクラス
    /// </summary>
    internal static class DatabaseTypeMetadataTableBinarySerializer
    {
        /// <inheritdoc cref="Serialize(ReadOnlyDatabaseTypeMetadataTable)"/>
        public static byte[] Serialize(this DatabaseTypeMetadataTable src)
            => Serialize((ReadOnlyDatabaseTypeMetadataTable)src);

        /// <summary>
        ///     <see cref="ReadOnlyDatabaseTypeMetadataTable"/> をバイナリ配列に変換する。
        /// </summary>
        /// <param name="src">処理対象</param>
        /// <returns><see cref="ReadOnlyDatabaseTypeMetadataTable"/> を変換したバイナリ配列</returns>
        public static byte[] Serialize(this ReadOnlyDatabaseTypeMetadataTable src)
        {
        ...
        /// </summary>
        /// <param name="src">処理対象</param>
        private static IEnumerable<byte> ToTypeDefinitionBinary(this ReadOnlyDatabaseTypeMetadataTable src)
        {
            var result = new List<byte>();
            ...
            // 項目名
            result.AddRange(
                src.FieldMetadataList.Select(x => x).SerializeFieldNames()
            );
            ...
            // 特殊指定
            result.AddRange(
                src.FieldMetadataList.SerializeSpecialSettingDescription()
            );
        ...
        /// </summary>
        /// <param name="src">処理対象</param>
        private static IEnumerable<byte> ToDataDefinitionBinary(this ReadOnlyDatabaseTypeMetadataTable src)
        {
            var result = new List<byte>();
            ...
            // 設定種別 & 種別順列
            result.AddRange(
                src.Settings[0].Settings.Select(value => value.Type).Serialize()
            );
        ...
        public static byte[] SerializeValuesDividedType(this ReadOnlyDatabaseTypeMetadataTable src)
